...
bool HitoriSolver::CheckSolution(QVector<QVector<int>> solution) {
    bool good = true;

    for (int i = 0; i < solution.size(); i++) {
        for (int j = 0; j < solution[i].size(); j++) {
            if (i > 0 && solution[i][j] == 1 && solution[i - 1][j] == 1)
                good = false;
            if (j > 0 && solution[i][j] == 1 && solution[i][j - 1] == 1)
                good = false;
            if (i < solution.size() - 1 && solution[i][j] == 1 && solution[i + 1][j] == 1)
                good = false;
            if (j < solution[i].size() - 1 && solution[i][j] == 1 && solution[i][j + 1] == 1)
                good = false;
        }
    }

    good &= (DisconnectedProcessing(solution.size(), solution));
    return good;
}
