...
void GameWindow::initBoardUI()
{
    ...
    ui->indicator->setStyleSheet("background-color: green;");
}
...
void GameWindow::checkSolution()
{
    ...
    QVector<QVector<int>> solution2 = solution;
    bool good = HitoriSolver::CheckSolution(solution2);
    if (!good)
        ui->indicator->setStyleSheet("background-color: red;");
    else
        ui->indicator->setStyleSheet("background-color: green;");
